...
def create_item(db: Session, data: schemas.ItemCreate) -> models.InventoryItem:
...
def get_dashboard_stats(db: Session, owner_id: str) -> schemas.DashboardStats:
    ...
    categories = (
        ...
        .filter(models.Category.inventory_id.in_(inv_ids))
        .all()
        if inv_ids
        else []
    )
    ...
    items = (
        db.query(models.InventoryItem)
        .filter(models.InventoryItem.category_id.in_(cat_ids))
        .all()
        if cat_ids
        else []
    )
